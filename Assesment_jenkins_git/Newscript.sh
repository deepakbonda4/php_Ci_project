...
git log --pretty=format:'%h|%an|%ae|%ad|%s|%n' --since="${daysToLog} days ago"

echo "CommitID,Author,AuthorEmail,CommitTime,CommitMessage" > ${WORKSPACE}/commits.csv

if [ ${WantOnlyValidCommit} == "YES" ]
then
     git log --pretty=format:'%h,%an,%ae,%ad,%s,%n' --since="${daysToLog} days ago" | grep JIRA >> ${WORKSPACE}/commits.csv
else 
     git log --pretty=format:'%h,%an,%ae,%ad,%s,%n' --since="${daysToLog} days ago" >> ${WORKSPACE}/commits.csv
fi

cp /home/ubuntu/Assesment_jenkins_git/htmlTable.sh ${WORKSPACE}

cd ${WORKSPACE}

cat commits.csv | bash htmlTable.sh -d "," -h "Commit Report" > index.html
